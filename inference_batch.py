...
while True:
	...
	img_bgr = cv2.imread("/home/pranoy/code/auto-transform/data/imgs/IMG_1906.JPEG")
	...
	img_h_1 = torch.tensor([img_h]).float().to(device)
	img_w_1 = torch.tensor([img_w]).float().to(device)

	focal_length_1 = estimate_focal_length(img_h, img_w)
	# bbox = detection_result[0][1:5]



	norm_img_1, center_1, scale_1, crop_ul, crop_br, _ = process_image(img_rgb, bbox)
	...
	norm_img_1 = torch.from_numpy(norm_img_1)
	norm_img_1 = norm_img_1.to(device)


	##-------------------

	# ret, img_bgr = vid.read()
	img_bgr = cv2.imread("/home/pranoy/code/auto-transform/data/imgs/IMG_1931.JPEG")
	draw_img = img_bgr.copy()
	# img_bgr = cv2.resize(img_bgr, (512, 512))
	...
	img_h, img_w, _ = img_bgr.shape
	img_h_2 = torch.tensor([img_h]).float().to(device)
	img_w_2 = torch.tensor([img_w]).float().to(device)

	focal_length_2 = estimate_focal_length(img_h, img_w)
	# bbox = detection_result[0][1:5]

	norm_img_2, center_2, scale_2, crop_ul, crop_br, _ = process_image(img_rgb, bbox)
	...
	# norm_img.shape torch.Size([32, 3, 256, 192])
	# center.shape torch.Size([32, 2])
	# scale.shape torch.Size([32])
	# img_h.shape torch.Size([32])
	# img_w.shape torch.Size([32])
	# focal_length.shape torch.Size([32])
	...
	center = torch.stack([center_1, center_2], dim=0).to(device)
	scale = torch.stack([scale_1, scale_2], dim=0).to(device)
	focal_length = torch.tensor([focal_length_1, focal_length_2]).to(device)
	img_h = torch.cat([img_h_1, img_h_2], dim=0).to(device)
	img_w = torch.cat([img_w_1, img_w_2], dim=0).to(device)
	...
	norm_img_2 = torch.from_numpy(norm_img_2)
	norm_img_2 = norm_img_2.to(device)

	norm_img = torch.stack([norm_img_1, norm_img_2], dim=0)
	...
	vertices = pred_output.vertices[1].unsqueeze(0)
	faces = smpl_model.faces[1]
	...
	landmarks = get_landmarks(pred_keypoints_2d)[0]
